...
// Pagination settings
$limit = isset($_GET['limit']) ? (int)$_GET['limit'] : 10; // Jumlah ruangan per halaman
...
// Fungsi untuk mendapatkan daftar ruangan dengan pagination
function getRuang($conn, $search = '', $offset = 0, $limit = 10) {
    $query = "SELECT * FROM ruang WHERE ruangan LIKE ? ORDER BY id_ruangan DESC LIMIT ?, ?";
...
function getTotalRuang($conn, $search = '') {
    $query = "SELECT COUNT(*) as total FROM ruang WHERE ruangan LIKE ?";
    $stmt = $conn->prepare($query);
    $searchParam = "%$search%";
...
$result = getRuang($conn, $search, $offset, $limit);
$totalRuang = getTotalRuang($conn, $search);

// Pastikan totalRuang tidak nol sebelum melakukan pembagian
$totalPages = ($totalRuang > 0) ? ceil($totalRuang / $limit) : 1; // Setidaknya ada satu halaman

// Membuat konten tabel ruangan
    ...
    while ($row = $result->fetch_assoc()) {
        $content .= '<tr>
            ...
            <td>
                ...
                <a href="hapus.php?id=' . $row['id_ruangan'] . '" class="btn btn-danger btn-sm">Hapus</a>
            </td>
        </tr>';
    }
...
$content = '
    ...
    <a href="tambah.php" class="btn btn-primary mb-3">Tambah Ruangan</a>
    <form method="POST" class="d-flex mb-3" role="search">
        <input type="search" name="search" class="form-control me-2" placeholder="Cari ruangan..." value="' . htmlspecialchars($search) . '">
...
if ($_SESSION['role'] === 'admin' || $_SESSION['role'] === 'manager') {
    $content .= '<a target="_blank" href="../laporan/laporan_ruang.php" class="btn btn-secondary mb-3 ms-2"><i class="bi bi-floppy me-2"></i>Laporan Ruang</a>
    </div>';
}
...
$content .= '<div class="d-flex justify-content-between align-items-center mb-3">
    <div>
        <span>Menampilkan ' . (($page - 1) * $limit + 1) . ' sampai ' . min($page * $limit, $totalRuang) . ' dari ' . $totalRuang . ' ruangan</span>
    </div>
    <div>
